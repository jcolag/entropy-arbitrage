...
One ship takes its name from the Greek mythological hero {% wiki Ajax_the_Great|Ajax|en %}.  The other takes its name from a variety of {% wiki HMS_Fearless|British|en %} and {% wiki USS_Fearless|American|en %} ships.
...
The show continues the tradition of nonsense star names.  {% wiki Tau|Tau|en %} (Τ/τ) and {% wiki Alpha|alpha|en %} (Α/α) both see use as Greek letters, and, unless someone out there diligently translates my blog posts for a wide audience, you probably already have some familiarity with {% wiki English_alphabet#Letter_names|C|en %} ("cee").  None of those provides a location.
...
I recap that here, because either the editor fell asleep when fixing Troi's line---"like quite wonderful" doesn't make sense, unless the character has transformed into a {% wiki Valley_girl|Valley Girl|en %} between scenes---or the writers meant her to feel aroused and decide against having her announce it.
