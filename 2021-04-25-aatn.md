...
It's later than I wanted, because everyone involved has apparently chosen to make it *more* complicated to run a Ruby on Rails app behind a web server---and secure it with {% wiki HTTPS|HTTPS|en %}---rather than easier.  I understand that everything needs to be more flexible, but the number of elements with different package names *and* non-working packages would be comedic, if I didn't know that millions of people rely on it.  Unlike most hassles, I will *not* turn this into a [tech tip](/blog/tag/techtips) post, because I expect that most of it will change too quickly to be useful to anybody but me.
