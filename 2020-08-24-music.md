...
labels: [blog, gitlab, library-update, uxuyu]
...
Along the same lines, my minimalist game [**Chasing Phantoms**](https://github.com/jcolag/ChasingPhantoms) now just has the pre-generated `readme.html` file included, so that I could note in the commit log that the game should be run from a server so that the {% wiki Cross-origin_resource_sharing|CORS|en %} only works when running under a web server.
