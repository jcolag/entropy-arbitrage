...
Pergium is original to this episode; Blish provides some loose hints about what it might be, but the closest thing that I can find that might have inspired the name is {% wiki Perg|Perg|en %}.

"Janus" doesn't appear to be based on any star, but is almost certainly named for Rome's {% wiki Janus|two-faced god|en %} of thresholds.  However, it's also a relatively common name and a {% wiki Janus_%28moon%29|moon|en %} of Saturn, so it could be named for any of those, instead.
...
Sirium is also original to the episode, as far as I can tell, though it happens to be the Roman name for {% wiki Mugwort|mugwort|en %} plants that I *assume* aren't being mined on Janus VI.  However, if it wasn't named for {% wiki Sirius|Sirius|en %}, a star less than nine light years away and seems to host a colony as early as 1996.  The trend of naming elements after the facilities that discover them, then, makes some sense.  Note that it's highly unlikely, but there's another reason that the name Sirius *might* be relevant to the episode mentioned in the credits, down at the [bottom of the page](#next).

If you'd like to pretend that history in the *Star Trek* franchise resembles our own, {% wiki Copernicium|Copernicium|en %} is the element discovered in 1996 in our world.
...
Scotty is going to make the same kind of point, but...it's a *pump*.  Humans have been moving water around since at least the {% wiki Archimedes'_screw|Archimedes screw|en %}, so surely, replacing it can't be too difficult.
...
**Credits**: The header image is [Aa large](https://commons.wikimedia.org/wiki/File:Aa_large.jpg) by the [United States Geological Survey](https://www.usgs.gov/), released into the public domain as a work of the United States Federal government.  Far from a Horta, it's an ʻaʻā flow from {% wiki K%C4%ABlauea|Kīlauea|en %}.  The Polynesians apparently refer to {% wiki Sirius|Sirius|en %} as ʻAʻā, interestingly enough, but the *Star Trek* franchise rarely refers to one of our nearest and most famous neighbors, primarily in off-handed references such as what we saw at the end of [*Arena*]({% post_url 2020-05-14-arena %}).
