...
For example, if you just want to collect tweets related to a hashtag, change the `stream` filter and don't `post()` anything.  Or, if you want to copy NPR and post {% wiki United_States_Declaration_of_Independence|The United States Declaration of Independence|en %} on our {% wiki Independence_Day_%28United_States%29|Independence Day|en %}, use the same structure, but *only* reply to the bot's own posts and post the first tweet outside the handler code.
