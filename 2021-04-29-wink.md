...
The name "Scalos" appears to be original to this episode, though at least {% wiki Scalos|software|en %} has been named for it since the episode aired.
...
Similarly, based on the weapons from each group that we see later, we can probably extrapolate that phasers aren't light-based weapons.  Speeding up doesn't cause the sorts of paradoxes that led to the formulation of {% wiki Special_relativity|special relativity|en %}, so we know that nobody is moving faster than light on the *Enterprise*.  And despite that, the phaser's beam is much slower than they are.
...
In prior episodes going back to [*The Man Trap*]({% post_url 2020-01-09-trek-01 %}), we've been given the impression that Federation culture is largely {% wiki Sex-positive_movement|sex-positive|en %}, though they're still trying to navigate how to talk about this when one of the participants happens to be in a long-term and committed monogamous relationship with someone else.
