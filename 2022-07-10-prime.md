...
Let's take this a step further, though.  Let's look at what we could build to enable a Free Culture ecosystem around it, a system that {% wiki Vertical_integration|vertically integrates|en %} most of the supply chain.
 ...
 * A case of {% wiki OpenCola_%28drink%29|OpenCola|en %}
...
Would a centralized production facility even make sense, though?  I ask, because facilities already exist with many manufacturing devices along the lines of what we list above:  {% wiki Hackerspace|Hackerspaces|en %} or makerspaces or whatever you'd prefer to call them.
...
If we want a fancy system, where---for fast turnaround items---we can deliver to someone in a park or on the beach, rather than dropping everything at front doors, this could use [WhatFreeWords](https://pballett.github.io/whatfreewords/), the Free Licensed counterpart to the {% wiki What3words|proprietary geocoding system|en %} that everybody basically ignores.  I believe that they have enough code that I could send someone to *encumber.wreak.wiggle* and have them end up in good seats at the {% wiki Tennessee_Amphitheater|Tennessee Amphitheater|en %}.
...
To this point, we have "Amazon, but not exploitative."  I like that well enough, but we have another pop culture retailer that manufactures and ships almost every product, {% wiki Acme_Corporation|Acme|en %}.  Can we stretch there?  Maybe, if we impose some strict limits.
