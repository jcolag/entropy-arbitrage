...
Because it's apparently a foreign concept to many people outside finance---where I spend basically none of my time, by the way---allow me to introduce and generalize the idea of {% wiki Return_on_investment|Return on Investment|en %} (RoI).  The easy definition is that it's the quotient when you divide the profit (the amount of money that an investment gives) by the amount of money that it cost (the initial investment).  Finance people use this to talk about "efficiency," and use it to compare opportunities, so that they can try to get the most money back for just having the money to invest.
...
The last item makes an important point about calculating investments:  If I choose to implement my own version of {% wiki Transport_Layer_Security|TLS|en %}, because I don't like that {% wiki OpenSSL|OpenSSL|en %} doesn't have enough maintainers to keep up with security vulnerabilities like {% wiki Heartbleed|Heartbleed|en %}---even if I consider my initial labor to be free---I would now have the responsibility to maintain my project and keep up with vulnerabilities.  That's now an ongoing cost that is part of the investment, but invisible at the start.
 ...
 * It's not *ready* for that role, but I can see a path where I use Bicker as the centerpiece of a business like {% wiki Disqus|Disqus|en %}.  It's a sleazy business model---all but requiring tracking people across the web---which is why I haven't taken the steps to make that easy, but it's certainly an option, if I ever need it.
...
Back then, as I said, reinventing wheels was routine.  I wrote *three* different Windows 3.1 programs to display an interface and connect it to library functionality based on a specification, because I couldn't reuse code written for other companies.  I've worked on software where the exception handling was written by hand, every time, using {% wiki Setjmp.h|`setjmp()`, `longjmp()`|en %}, and the {% wiki C_preprocessor|C Preprocessor|en %}.  Once, I needed to implement text input and output in assembly language, because the client commissioned a custom {% wiki BIOS|BIOS|en %} from a contractor that turned out to be broken.

You can find many terminal-based tools that simulate a multi-window desktop, and most of those were written from scratch.  I once interviewed with a company where my prospective colleague needed to implement the {% wiki Internet_protocol_suite|Internet protocol suite|en %}.  Just about everybody working for more than fifteen years has needed to write custom code to print dates and times.  The same goes for logging, which is why it's hard to find two programs from more than a few years ago with anything like similar log formats.
...
[^1]: The gavvo is the national currency of {% wiki Graustark|Graustark|en %}, because we wouldn't want to give the impression that this only works for dollars...
