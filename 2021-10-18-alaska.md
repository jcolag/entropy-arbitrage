...
labels: [doritis-onomaton, intern, rust]
...
Today is the {% wiki Alaska_Day|anniversary of the transfer of Alaska|en %} from the Russian Empire to the United States.  Yes, that's the best holiday that I could find.  It's less depressing than our taking over Puerto Rico.
...
There's also the start of a {% wiki Berkeley_sockets|socket|en %}-based API that listens on (TCP) port 48813 and responds---if not necessarily in a useful way---to input.  While not yet useful, it should be easy to expand into a server that returns search results.
