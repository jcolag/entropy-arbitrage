...
labels: [basic, blog, c, library-update, social-media, tool-trunk]
...
Today marks the 133<sup>rd</sup> anniversary of the admission of {% wiki Wyoming|Wyoming|en %} to the United States.  Yes, we have so few holidays of interest for today, that I had to resort to another state anniversary.
...
For the base language, I chose an old-style {% wiki BASIC|BASIC|en %}, for a handful of reasons.
...
The code itself?  It looks like it comes to about two thousand lines of C, with a hand-coded {% wiki Recursive_descent_parser|recursive-descent parser|en %} and some dynamic data tables.  The formatting that I used back then boggled the mind.
