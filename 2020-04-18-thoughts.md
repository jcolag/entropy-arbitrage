 ...
 * As more services are hosted in federated and distributed networks, **auditing** is going to be critical, whether that's supporting specific {% wiki Right_to_be_forgotten|rights to be forgotten|en %} or preventing discrimination around events.  A centralized site can be sued, if there's evidence of illegal activity, and a site run by a corporation can be monitored, but (for example) a Mobilizon instance could plausibly exclude women from job fairs without any easy recourse.
