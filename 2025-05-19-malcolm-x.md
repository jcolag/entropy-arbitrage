...
Today celebrates {% wiki Malcolm_X_Day|Malcolm X Day|en %}, honoring the civil rights leader.  Given that most discussions of the 1960s United States Civil Rights Movement tend to downplay him for calling for sometimes-literal revolutionary change, the holiday hasn't seen wide adoption, but that never stopped me...
...
RSS Guard uses {% wiki Unix_time|UNIX time|en %} for the dates, so that required some quick work to get the numbers, but I go through all this to finally mention that the repository now includes and uses a script to pull the month's saved articles from RSS Guard, replacing the prior script.
