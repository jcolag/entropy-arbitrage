...
It turned out to be a strange experience, because the company's needs changed as we made it through the course. I started out working through the Win32 API in C, but the company decided they wanted to use {% wiki Microsoft_Foundation_Class_Library|MFC|en %}, which I had avoided learning because it revolved around coding behind an interface and so seemed clunky at the time...even though that's obviously more or less standard procedure, today.
