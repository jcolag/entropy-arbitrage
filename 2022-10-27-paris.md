...
Sarona probably takes its name from the communities in {% wiki Sarona_%28community%29,_Wisconsin|Wisconsin|en %} and {% wiki Sarona_%28colony%29|Israel|en %}.

 > **DEAN**: I took advantage.

People still {% wiki Fencing|fence|en %}, and you might notice that not much about it has changed, other than the computer scoring the match on---for some reason---one large monitor for each participant.
...
You might recognize Jenice as {% wiki Michelle_Phillips|Michelle Phillips|en %}, singer for {% wiki The_Mamas_and_the_Papas|the Mamas and the Papas|en %}.
...
Assuming that I got the intended spelling, they probably named the system for the {% wiki Ilocos_Region|Ilocos|en %} region in the Philippines, where the {% wiki Ilocano_people|Ilocano|en %} predominantly live.
