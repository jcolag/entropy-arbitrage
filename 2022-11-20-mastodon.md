...
To make this work, head over to the *Preferences*, then click on *Profile*, and *Appearance*.  Near the bottom of that page, you'll see some inputs with the useless-sounding label of "Profile metadata."  Despite the name, you use them to identify yourself across the Internet.  For example, the top line of my "metadata" says that I have a "Personal Website" (type that into the left-hand column) of `https://john.colagioia.net` (in the right-hand column).
...
In other words, a hypothetical `@potus@whitehouse.gov` account has significantly more credibility than a `@potus@random.website.invalid` account with a verified link to <https://whitehouse.gov>.  That doesn't apply to everyone, but if you do happen to run an organization, want your people to have a presence on Mastodon, and want them to act in some official capacity, give some thought to it.
