...
labels: [blog, library-update, mini-server, newsletter, notoboto, stenotype]
...
Today marks {% wiki Asteroid_Day|International Asteroid Day|en %}, commemorating the anniversary of the {% wiki Tunguska_event|Tunguska event|en %} that has driven {% wiki Tunguska_event_in_fiction|so much science fiction|en %} over the last century or so.  In practice, it serves more as an excuse to talk about {% wiki Asteroid_impact_avoidance|impact avoidance|en %}.
...
Following up on [last week's developer diary post]({% post_url 2025-06-23-midsummer %}), resetting the {% wiki Nonvolatile_BIOS_memory|BIOS memory|en %} got the newer laptop past the first hurdle of at least doing *something* on boot, it doesn't seem to get past turning on the Caps Lock light, before stalling again.  No fan, no screen backlight, nothing.
