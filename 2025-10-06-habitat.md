...
Today, we celebrate {% wiki World_Habitat_Day|World Habitat Day|en %}, reflecting on the essential right to adequate shelter.  Themes have ranged from sheltering the unhoused, to safer cities, to the gender gap in local governance, and so forth, with occasional nods at trying to head off urban sprawl.
...
In addition, we celebrate and affirm the lives of people living with cerebral palsy for---to nobody's surprise, I imagine---{% wiki World_Cerebral_Palsy_Day|Cerebral Palsy Day|en %}.  And we also celebrate student-run broadcast media of greater variety than the name {% wiki College_Radio_Day|College Radio Day|en %} implies.
...
While I never finished this, largely because {% wiki Cross-origin_resource_sharing|Cross-Origin Resource Sharing|en %} (CORS) rules made it messier than I wanted to deal with during development, I have copied the serverless application over to Codeberg.
...
[^pDTnMc]:  Rube, as in {% wiki Rube_Goldberg|Goldberg|en %}, the cartoonist, not [Ruby](https://www.ruby-lang.org/en/) or any [yokel](https://en.wiktionary.org/wiki/rube), the last allegedly deriving from the disproportionate number of men named Reuben in small towns.
...
And one last little programming language moving to Codeberg for the week, Prim *only* provided access to some minimal set of {% wiki Primitive_recursive_function|primitive recursive functions|en %}, though I apparently never got to the *recursive* part.  And the compiler also inexplicably generates code for a closed-source C-like structured assembly language.
