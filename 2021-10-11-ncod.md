...
So far, I've been able to get notifications on modified files, configure the program based on a settings file, and store the last known update and a primitive {% wiki Inverted_index|inverted index|en %} into an [SQLite](https://sqlite.org/index.html) database stored with the settings.
...
The "modified" time is seconds since the {% wiki Unix_time|UNIX epoch|en %} (GMT), so that file was last changed on Wednesday night---October 5<sup>th</sup>, which probably shouldn't be surprising---my time.  Then, we have our word stems.
