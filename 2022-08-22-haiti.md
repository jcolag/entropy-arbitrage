...
Today marks the 231<sup>st</sup> anniversary of the start of the {% wiki Haitian_Revolution|Haitian Revolution|en %}, among other things, history's largest and most effective slave revolt.  While generally considered one of the most important parts of the {% wiki Atlantic_Revolutions|Atlantic Revolutions|en %}, many at the time and since have dismissed its importance, given attitudes in the Western world's elite about slavery.
...
However, Ubuntu will now only ship Firefox as a {% wiki Snap_%28software%29|Snap|en %}.  Now, don't get me wrong, here.  I don't have a problem with Snaps, or Ubuntu "forcing" users to use them, as if they haven't "forced" me to use the {% wiki Deb_%28file_format%29|deb|en %} format.  It frustrates me that I need to search five different places (plus hunt for source code) for applications, these days, instead of just installing things, but security requires some trade-offs, and those tradeoffs can get pretty weird.  This package has caused me two critical kinds of grief, however.

### Everything but the Firefox Sync

First and immediately clear, this new build of Firefox had no intentions of helping me pick up where I left off.  I can kinda-sorta access the list of tabs synchronized through {% wiki Firefox_Sync|Firefox Sync|en %}, but it has somehow opted *not* to automatically carry over my configuration and session cookies, though it somehow cleanly installed all my *plugins* in the proper states.  In some cases, it doesn't seem capable of retaining sessions that I know worked fine days before.
...
**Credits**:  The header image is [Combat et prise de la Crête-à-Pierrot](https://commons.wikimedia.org/wiki/File:Haitian_Revolution.jpg) by {% wiki Auguste_Raffet|Auguste Raffet|en %}, long in the public domain.
