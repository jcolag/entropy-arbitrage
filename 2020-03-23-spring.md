...
Boostnote stores its "folders" as names in a {% wiki JSON|JSON|en %} configuration file, with each note stored as a [CSON](https://www.lee-dohm.com/big-book-of-atom/1-introduction/30-coffeescript-object-notation.html) file that refers to its folder (among other things) and stores the note text as [Markdown](https://www.markdownguide.org/).
...
**Credits**:  The header image is [Orchard in Spring](https://commons.wikimedia.org/wiki/File:Alfred_Sisley_047.jpg) (1881) by {% wiki Alfred_Sisley|Alfred Sisley|en %}.
