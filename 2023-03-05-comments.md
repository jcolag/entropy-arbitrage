...
However, since almost every project uses some form of {% wiki Version_control|version control|en %}, your *commit message* should absolutely reference the relevant ticket, which joins the two systems through metadata.
...
If we run this code to sort things, we don't need any comments.  The code sorts.  It has a label of {% wiki Quicksort|quicksort|en %}, which people can look up to learn the algorithm.
