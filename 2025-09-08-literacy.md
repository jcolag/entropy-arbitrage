...
labels: [blog, codeberg-move, codidact, ham-newsletter, mini-server, ollama, social-media]
...
Today marks {% wiki International_Literacy_Day|International Literacy Day|en %}, aiming to highlight the importance of reading, increasingly on a societal level.  Around one in five adults currently can't read, with regional illiteracy rates ranging from Burkina Faso's 12.8% to South Asia's 58.6%, correlating strongly with both poverty and misogyny, for reasons that we can probably all guess.
...
The scripts to generate and send the newsletter---out on Saturday for those of you subscribed on Mailchimp[^3], and tomorrow morning for those of you taking the [Buy Me a Coffee](https://buymeacoffee.com/jcolag) route---also now live on Codeberg.
...
Actually, I *did* add an additional script for my own use, but it more properly belongs to the blog rather than the newsletter, since I use it to streamline the [Social Media Roundup](/blog/tag/link-dump) posts, so you won't find it in the repository, yet.  It adapts the existing script to get the saved articles from Fresh RSS, but instead of getting all articles for the previous month, it grabs those from Monday through Friday, so that I can pick them off faster for my post than clicking through on Fresh RSS itself.
