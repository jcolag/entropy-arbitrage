...
While it probably doesn't complete the process, but the code has dropped the attempt at direct manipulation via {% wiki JSON-LD|JSON-LD|en %} and replaced that nonsense with the creation of a {% wiki JSON_Web_Token|JSON Web Token|en %}.  Creating the signature from the JWT creates a binary mess that we can't add as metadata, so---unless I misread the specification---it builds a hexadecimal representation.
