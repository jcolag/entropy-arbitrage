...
labels: [code-server, data-recovery, inf-cloud, forgejo, jellyfin, mini-server, nginx, notoboto, radicale, snappy-mail, stenotype, tcl]
...
For those planning to do something similar now that I brought it up, make sure that you disable or uninstall [ASF `httpd`](https://projects.apache.org/project.html?httpd-http_server)[^ynfKtM], which Raspbian may have installed by default.

[^ynfKtM]:  Have they given the server an official not-deliberately-offensive name, yet...?
