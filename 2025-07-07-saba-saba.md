...
[^1]:  I didn't find it clear enough documentation anywhere, so I'll explain here.  On the surface, you use Snappy Mail by putting your login credentials into their authentication screen, letting it scrape down your e-mail, and present that account with a lightweight interface, rather than having accounts of its own.  However, once you do that for the first account, you can add additional mail accounts that you switch between from a drop-down menu.  That first e-mail address, then, gets treated as vaguely similar to a user account.
