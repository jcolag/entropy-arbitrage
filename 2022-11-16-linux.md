...
Our house started with an {% wiki Atari_8-bit_family|Atari 400|en %}.  Most of the family and our visitors came down with (ahem) "{% wiki Pac-Man|Pac-Man|en %} Fever," wearing out a long series of those cheap joysticks, but I fell in love with the neglected {% wiki BASIC|BASIC|en %} cartridge that (if I remember correctly) came in the computer's box at the time.

A few years later, we moved to the new {% wiki Commodore_64|Commodore 64|en %}.  I sometimes joke that I learned to read in order to plow through **The Commodore 64 Programmer's Reference Guide**, a comb-bound book from the manufacturer that detailed that version of Microsoft BASIC, the graphics model, the sound model, Motorola 6502 machine language, input/output programming, and then a variety of tables and schematics.  I still have a copy, though I forget whether I've owned that particular copy since 1982 or picked it up used to replace the original.
...
Due to the flexibility, not to mention the color graphics and multi-voice audio, I assumed that we had bet on "the winner" of the home computer competition, and felt sorry for the people who bought an {% wiki IBM_Personal_Computer|IBM PC|en %} or one of the so-called {% wiki IBM_PC_compatible|clones|en %}.  Oh, you can produce sound on a tinny speaker and show {% wiki Color_Graphics_Adapter|blocky graphics in four unnatural colors|en %}, on more expensive hardware?  Amazing...
...
We got a computer running MS-DOS and some bizarre graphical environment that made {% wiki GEM_%28desktop_environment%29|GEM|en %} look like it came from the distant future.  We eventually got rid of that, working in DOS until prices on {% wiki Windows_3.1x|Windows 3.1|en %} came down, and...
...
First, the campus (other than administrative tasks) ran primarily on various versions of {% wiki SunOS|SunOS|en %} UNIX, with {% wiki Oracle_Solaris|Solaris|en %} poking its way into the field.  I didn't like MS-DOS or Windows, but I could get behind UNIX, goofy name aside.  If you find old e-mail discussions archived where I participated, the "{% wiki Johannes_Kepler|Kepler|en %}" and "{% wiki Rendezvous_with_Rama|Rama|en %}" machines in those old addresses ran Solaris.

A quick aside for the confused:  The story goes that the name "UNIX" plays on the system's similarity to the earlier {% wiki Multics|Multics|en %} operating system, but also an off-color joke about the {% wiki Eunuch|class of servants|en %} in various cultures, mutilated as a show of power.  Every once in a while, I run across someone else who insists on calling himself "Unix-Man," often complaining about his dating life, and...they must do that as a comedy bit, right?  They have to know what that sounds like...

Anyway, not long after, students in the dorm started running {% wiki Linux|Linux|en %} on their computers.  And you would think that, at this point in the story, I tell you that I fell in love with the system, and have used it ever since.
...
Mind you, I did loosely keep up with Linux for a few years, through the occasional book/CD set from {% wiki Walnut_Creek_CDROM|Walnut Creek CDROM|en %}, but didn't take any of those versions seriously, and mostly focused on the software.  I don't remember ever installing {% wiki Slackware|Slackware|en %}---they distributed it---but may have tried it on a virtual machine.  But I'd also occasionally use {% wiki Knoppix|Knoppix|en %} as a "rescue disk" when something went wrong on Windows.
