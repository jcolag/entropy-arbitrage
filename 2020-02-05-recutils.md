 ...
 * I can quickly search anything I'm working on with a tool like {% wiki Grep|`grep`|en %}.
 * I'm not tied to specific applications.
 * It's easier to convert to another format, making it closer to future-proof.

For example, [this blog](https://github.com/jcolag/entropy-arbitrage), [my novel](https://github.com/jcolag/silver-bat-01-seeking-refuge), and my personal notes are generally all written as [Markdown](https://www.markdownguide.org/) files.  I store most of my spreadsheets as sets of {% wiki Comma-separated_values|CSV|en %} files, unless the formatting or calculated cells are important.  In the rare cases I need something like a flow chart, it's in [pic](http://floppsie.comp.glam.ac.uk/Glamorgan/gaius/web/pic.html) or---increasingly---[Mermaid](https://mermaid-js.github.io/mermaid/#/) format.
 ...
 * Separate the record types into different files, while specifying *all* of them with a wildcard (such as "`*.rec`") whenever working in situations that require more than one record type, like a {% wiki Relational_algebra#Joins_and_join-like_operators|Join|en %}.
...
And no, I've never heard of the {% wiki Toyota_concept_vehicles_%282000%E2%80%932009%29#Pod|Toyota Pod|en %} before now.  It came generated with the fictional person's dossier.  It was a concept car that appears to be the automotive equivalent of the early {% wiki Smart_refrigerator|smart refrigerators|en %} that nobody would really want, but is easy to explain to people.
 ...
 * **`mdb2rec`** converts a {% wiki Microsoft_Access|Microsoft Access|en %} database (or a single table) into records.
