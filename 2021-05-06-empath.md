...
The Ritter scale seems to be original to the episode.  In reality, we measure {% wiki Solar_flare|solar flares|en %} in terms of energy and X-ray flux per square meter.

 > **OZABA**: In his hand are the deep places of the earth. Psalm ninety-five, verse four. Looks like he was listening.

That is, in fact, a line from {% wiki Psalm_95|Psalm 95|en %}...not that it's really relevant to the story beyond them literally being underground.

Meanwhile, it's perhaps noteworthy that Ozawa is a Japanese name, whereas actor {% wiki Davis_Roberts|Davis Roberts|en %}---probably mainly famous for his care in choosing roles that were more respectful than the normal fare for Black actors---certainly isn't.  That's admittedly not much, but it implies that many of the character names might not reflect the ethnicity of the characters, except in cases where the ethnicity has been mentioned in dialogue.
...
Scott is referring to the {% wiki Parable_of_the_Pearl|Parable of the Pearl|en %}.  It's probably notable how confused everybody is by the reference, since that implies that it's not commonly known and that it's not an obvious metaphor.  That's especially true, given how poor the metaphor is for the situation; nobody invests everything into acquiring Gem.

## Blish Adaptation

This adaptation comes from **Star Trek 10**.  The solar flare is measured on the "Van Allen Scale"---presumably a reference to the {% wiki Van_Allen_radiation_belt|Van Allen radiation belt|en %}---but it's otherwise basically the episode as aired.
