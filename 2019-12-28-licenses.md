...
I completely bought into the nonsense of the GPL (and other {% wiki Copyleft|copyleft|en %} licenses) being "viruses" that "infect" software.  I also bought into nonsense that the only reasons to publish software were either to make money or to allow someone else make money.  And I probably didn't value my time, thinking of it as free; either that, or I didn't put value on the time of people who created software I used.
...
Another problem is that, traditionally, contracts require {% wiki Consideration|consideration|en %} to be valid.  The "permissive" licenses don't have *any* consideration, only offering the software.  You might argue that something like the BSD license is a {% wiki Deed|deed|en %} rather than a contract, but it doesn't say so and isn't witnessed, so it would also be a poor deed.  So, whereas the GPL has [been upheld in court](https://www.techdirt.com/articles/20170515/06040337368/us-court-upholds-enforceability-gnu-gpl-as-both-license-contract.shtml) in several countries, I'm not convinced that BSD, MIT, and Apache have the same strength, meaning that the developers and the users may not be protected.
...
When {% wiki Garrett_Hardin|Garrett Hardin|en %} wanted to talk about the potential problems of overpopulation in 1968, coining the phrase *Tragedy of the Commons*, he made two points relatively clear.
...
Or, as philosopher {% wiki Georg_Wilhelm_Friedrich_Hegel|Georg Wilhelm Friedrich Hegel|en %} and **Communist Manifesto** co-author {% wiki Friedrich_Engels|Frederick Engels|en %} put it,
...
And returning to the idea of consideration in contract law, if you don't work to maintain the commons, is there a possible future where {% wiki FreeBSD|FreeBSD|en %} developers decide that the {% wiki BSD_licenses#2-clause|BSD License|en %} does not represent a permanent grant of any rights, since it doesn't refer to any such thing, and sue Apple for copyright infringement?  I don't think it would happen, but it seems possible, especially in a world where Apple ceases all support because they got what they wanted.
...
The last big reason that has convinced me to change my mind is that, at least in the software world, almost all open source code can be used in a project licensed under the AGPL.  There are exceptions, of course, especially among the competing copyleft licenses such as the {% wiki Eclipse_Public_License|EPL|en %}.  But for the most part, if software has an [OSI-approved license](https://opensource.org/licenses), the AGPL's terms are a superset of its requirements.
...
This is why I would encourage everybody to avoid involvement in projects with {% wiki Contributor_License_Agreement|Contributor License Agreements|en %}, since the agreements exist partly to eliminate the need for permission in relicensing.
