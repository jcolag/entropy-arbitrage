...
Far worse than arguments over lunar holidays is that everybody gets {% wiki Pi_Day|Pi Day|en %} wrong.  They're all lying to you.  It's *tomorrow*.  You can trust me, though.
...
Better than that---for very niche definitions of "better"---this works for any physical constant, no matter how big or small, with only {% wiki Scientific_notation|scientific notation|en %} and uneven distribution across the calendar requiring any subjective judgment.
...
Hey, why is the {% wiki Plastic_number|plastic number|en %} represented by rho (ρ)?  Ah.  In French, it's *le nombre **r**adiant*.  That makes significantly more sense.

## Bonus:  Change of Base

Now that we've done all that admittedly-weird work, we now have a much simpler algorithm to change the {% wiki Radix|radix|en %} (or base) of numbers than is generally taught.  All those times in the code where we multiply and yank off the whole number are the key.  For integers, we need to...
