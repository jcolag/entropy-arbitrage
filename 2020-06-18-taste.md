...
This is an interesting, if unlikely, twist, right off the top.  {% wiki NGC_321|NGC 321|en %} is real, and a *galaxy*, which I suppose is just an extremely large and distant star cluster, in a way.  It's found in the constellation {% wiki Cetus|Cetus|en %}, perhaps most interesting because we recently had references to α Cet and τ Cet, last week in [*Space Seed*]({% post_url 2020-06-11-seed %}).

If we were to imagine that this is meant to signal some intentional (gasp!) continuity between episodes, that could indicate that Eminiar is just a star in the same general direction and possibly in the same range, such as {% wiki Phi2_Ceti|Phi<sup>1</sup> Ceti|en %}/φ<sup>1</sup> Cet.  A little over two hundred light years away, slightly smaller right ascension, larger-but-similar declination, and from Earth looks to be part of a set of four "nearby" stars at different distances out, which is what the superscript numbers are for, in order of apparent brightness.
...
For the record, {% wiki Low_Earth_orbit|low Earth orbit|en %} is around two thousand kilometers above the surface, whereas Earth's {% wiki Exosphere|exosphere|en %} runs from seven hundred kilometers to around ten thousand kilometers, and it's generally accepted that the layer doesn't have enough pressure to transmit sound.
...
We could laugh at the absurdity of someone believing that their job title protects them from violence, but during what we're apparently calling the {% wiki George_Floyd_protests|George Floyd Protests|en %} (even though Floyd was only one of many black people that triggered the cascade of concern that still continues), we saw multiple journalists write about how police officers shoved them, even after they showed their press passes, as if the ID card had magic powers.
