...
This seems like a strange position, in a science fiction universe, because the idea has a long tradition, both inside and outside fiction.  {% wiki Christina_of_Sweden|Christina of Sweden|en %} allegedly challenged {% wiki Ren%C3%A9_Descartes|Descartes|en %} to build a reproducing clock.  Machines that produce copies of themselves show up in {% wiki Erewhon|**Erewhon**|en %}.  And a {% wiki Von_Neumann_universal_constructor|fairly intricate vision|en %} came from John von Neumann in the 1940s.  By the time this episode aired, K. Eric Drexler had even published the {% wiki Gray_goo|gray goo|en %} thought experiment.  Even [*Lonely among Us*]({% post_url 2022-06-30-lonely %}), in this series, suggested that people can use the transporters for manufacturing.  None of these examples presume that the machines would have anything like life.
...
I guess that I should fact-check this, while I have it in front of me.  Rather than "transistor material," the term that they want is {% wiki Semiconductor|semiconductor|en %}, any of which one could use to make transistors, and which have other uses.  Industry has used {% wiki Gallium_arsenide|gallium arsenide|en %} in infrared {% wiki Light-emitting_diode|light-emitting diodes|en %}, but also uses it for solar panels and as a general semiconductor.  Cadmium selenide sulfide doesn't exist, but {% wiki Cadmium_selenide|cadmium selenide|en %}---another semiconductor---has luminescent qualities and has value when mixed with {% wiki Cadmium_sulfide|cadmium sulfide|en %}, for its transparency to infrared light.  And salt water does conduct electricity.  In other words, their analysis has sloppy flaws, probably because the writers didn't pay close attention when someone explained this to them, rather than due to shoddy science education in Federation schools.

 > **COMPUTER**: Probability positive.
 >
 > **WORF**: I wasn't asking you.

Mostly, I want to draw attention to how, following the lack of status and respect held by characters played by Black men, Worf takes his frustration out on the feminine-coded computer, reflecting the {% wiki Female_gendering_of_AI_technologies|role of feminized voice assistants|en %} in our world.
