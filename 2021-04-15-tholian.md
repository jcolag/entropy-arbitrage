...
This implies that the crew doesn't actually speak English---despite the evidence we've seen in the series so far that they do---and, based on the name, might speak something based on the original {% wiki Interlingua|Interlingua|en %}.  But whether it's English or Interlingua, we have another situation where Federation citizens expect that aliens will learn their language, instead of using the translation device from [*Metamorphosis*]({% post_url 2020-10-08-meta %}).

Other than that, the nerve gas is named "chlortheragen," suggesting that it's a chemical structure centered on a {% wiki Chlorine|chlorine|en %} atom.  It's still Klingon, though, so that doesn't really matter.
