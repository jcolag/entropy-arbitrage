...
labels: [blog, github-profile, newsletter]
...
Yesterday was {% wiki Friendship_Day|Friendship Day|en %}, though almost nobody actually celebrates it on that day, and is a *literal* {% wiki Hallmark_holiday|Hallmark Holiday|en %}, created by the company's founder Joyce Hall.  It was also, apparently, International Forgiveness Day, though I can't find a legitimate reference for it.  This is also the best of the August holidays, so they're going to slide awkwardly downhill from here.
...
There wasn't much code involved, but I messed around with the analytics system and made sure everything was {% wiki General_Data_Protection_Regulation|GDPR|en %}-compliant.  I think the law is ham-fisted, myself, and don't need to care what happens in the EU, but if it makes someone more comfortable spending time on the site, it's worth the trouble.
...
You can find the code I use for generating everything in the [**Ham Newsletter**](https://github.com/jcolag/ham-newsletter) repository, named for {% wiki Ham_(chimpanzee|the first chimp in space|en %}, given the service that maintains the list.
