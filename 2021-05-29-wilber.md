...
Speaking of unidentified characters, the conference call comic shows many mascots that I can identify---{% wiki FreeDOS|Blinky|en %}, {% wiki OpenBSD|Puffy|en %}, {% wiki Tux_%28mascot%29|Tux|en %}, Wilber, {% wiki GNU_Project|the gnu|en %}, {% wiki BSD_Daemon|Beastie|en %}, and {% wiki KDE|Konqi|en %}---but there's also a bunch that almost seem like they were meant to be placeholders that were never replaced with real mascots.  The monkey has a [Blender](https://www.blender.org/) logo, so I have to assume that there's some connection that I'm just not aware of.  The fox is maybe intended to represent [Firefox](https://www.mozilla.org/en-US/firefox/new/), even though {% wiki Mozilla_%28mascot%29|Mozilla|en %} would probably make more sense.  I can't even imagine who the lady goat(?) with the glasses is supposed to be.
